import React from "react";

const MyBookings = () => {
  return (
    <div className="p-4 flex justify-center items-center gap-10 flex-wrap">
      <div className="min-w-2xl text-white bg-neutral-200 rounded-md p-4">
        <div className="flex justify-between">
          <div className="flex justify-center flex-col items-center gap-3">
            {" "}
            <img
              className="size-40"
              src="data:image/png;base64,"
              alt="qr code"
            />
            <button className="px-5 py-2 bg-cyan-700 text-white rounded-md active:scale-95 hover:bg-cyan-800">
              {" "}
              Delete
            </button>
          </div>
          <div className="flex gap-2 flex-col">
            <div>
              <h3 className="text-black text-lg"> Ticket id:</h3>
              <p className="text-cyan-600 font-bold">
                {" "}
                ticket id dalna baki hai
              </p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Location:</h3>
              <p className="text-cyan-600 font-bold">
                {" "}
                Motera Stadium Ahmeabad
              </p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Price:</h3>
              <p className="text-cyan-600 font-bold">₹1,499</p>
            </div>
          </div>
          <div className="flex gap-2 flex-col">
            <div>
              <h3 className="text-black text-lg"> Date & Time:</h3>
              <p className="text-cyan-600 font-bold"> 2025-03-15 , 19:32 </p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Booking Date:</h3>
              <p className="text-cyan-600 font-bold"> 23/09/2024</p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Title:</h3>
              <p className="text-cyan-600 font-bold">Indian Premier League</p>
            </div>
          </div>
        </div>
      </div>

      <div className="min-w-2xl text-white bg-neutral-200 rounded-md p-4">
        <div className="flex justify-between">
          <div className="flex justify-center flex-col items-center gap-3">
            {" "}
            <img
              className="size-40"
              src="data:image/png;base64,"
              alt="qr code"
            />
            <button className="px-5 py-2 bg-cyan-700 text-white rounded-md active:scale-95 hover:bg-cyan-800">
              {" "}
              Delete
            </button>
          </div>
          <div className="flex gap-2 flex-col">
            <div>
              <h3 className="text-black text-lg"> Ticket id:</h3>
              <p className="text-cyan-600 font-bold">
                {" "}
                ticket id dalna baki hai
              </p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Location:</h3>
              <p className="text-cyan-600 font-bold">
                {" "}
                Motera Stadium Ahmeabad
              </p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Price:</h3>
              <p className="text-cyan-600 font-bold">₹1,499</p>
            </div>
          </div>
          <div className="flex gap-2 flex-col">
            <div>
              <h3 className="text-black text-lg"> Date & Time:</h3>
              <p className="text-cyan-600 font-bold"> 2025-03-15 , 19:32 </p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Booking Date:</h3>
              <p className="text-cyan-600 font-bold"> 23/09/2024</p>
            </div>
            <div>
              <h3 className="text-black text-lg"> Title:</h3>
              <p className="text-cyan-600 font-bold">Indian Premier League</p>
            </div>
          </div>
        </div>
      </div>

      
    </div>
  );
};

export default MyBookings;
